...
pipwerks.SCORM = {									//Define the SCORM object
    ...
    sessionTime:{ startTime: new Date().getTime() }	//PG Addition: To manage session time from when page is viewed to unloaded
};

function unloadPage(){

	var sessionTime = pipwerks.UTILS.getSessionTime(),
	...
	if(scorm.connection.isActive){
        ...
        if(API){
    		switch(scorm.version){
				case "1.2" : pipwerks.SCORM.set("cmi.core.session_time", sessionTime); break;
				case "2004": pipwerks.SCORM.set("cmi.session_time", sessionTime); break;
			}
		}
	}
	// Commit changes
...
/* ------------------------------------------------------------------------
   pipwerks.UTILS.getSessionTime()
   Works out the session time from the start of the page
   
   (This is a function created by Paul Gray)
--------------------------------------------------------------------------- */

pipwerks.UTILS.getSessionTime = function(){

	var scorm = pipwerks.SCORM,
	startTime = pipwerks.SCORM.sessionTime.startTime,
	convertTotalSeconds = pipwerks.UTILS.convertTotalSeconds,
  ...
  //LMSCall("setValue", "cmi.core.session_time", "cmi.session_time", formattedTime , formattedTime);
}

pipwerks.UTILS.convertTotalSeconds = function(ts){
   var sec = (ts % 60);
   ...
   var rtnVal = hour + ":" + min + ":" + strSec;  // "PT" + hour+"H"+min+"M"+strSec+"S"; 
